...
def try_screenshot(driver, ip, port, scheme, output_folder, log_entries, json_entries, ignore_ssl_errors, timeout=10, retries=1):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scan IPs for websites and capture screenshots.")
    ...
    parser.add_argument("--timeout", type=int, default=10, help="Timeout for each page load in seconds (default 10)")
    parser.add_argument("--retries", type=int, default=1, help="Number of retries per page if first load fails (default 1)")
    parser.add_argument("--no-ssl-check", action="store_true", help="Disable SSL certificate validation")
    parser.add_argument("--zip", action="store_true", help="Zip output folder after scan")
    parser.add_argument("--json", action="store_true", help="Save scan results as structured JSON")
    parser.add_argument("--input", help="Path to file containing list of IPs or CIDR ranges (one per line)")
...
def main_scan(target_ips, ports, timeout=10, retries=1, ignore_ssl_errors=False, zip_output=False, save_json=False):
    ...
    try:
        for ip in target_ips:
            ...
            for port in open_ports:
                if try_screenshot(driver, ip, port, 'https', output_folder, log_entries, json_entries, ignore_ssl_errors, timeout, retries):
                    continue
                try_screenshot(driver, ip, port, 'http', output_folder, log_entries, json_entries, ignore_ssl_errors, timeout, retries)
...
def main():
    ...
    main_scan(target_ips, ports, timeout, retries, ignore_ssl_errors, zip_output, save_json) 
